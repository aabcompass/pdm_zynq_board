...
--        others  - reserved
-- 0x14 : reserved
-- 0x18 : Data signal of CH_INFO
--        bit 7~0 - CH_INFO[7:0] (Read/Write)
--        others  - reserved
-- 0x1c : reserved
...
architecture behave of scurve_adder_CTRL_BUS_s_axi is
    ...
    constant ADDR_CH_INFO_DATA_0 : INTEGER := 16#18#;
    constant ADDR_CH_INFO_CTRL   : INTEGER := 16#1c#;
    ...
    signal int_CH_INFO         : UNSIGNED(7 downto 0);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CH_INFO_DATA_0 =>
                        rdata_data <= RESIZE(int_CH_INFO(7 downto 0), 32);
                    when others =>
                        rdata_data <= (others => '0');
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    CH_INFO              <= STD_LOGIC_VECTOR(int_CH_INFO);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CH_INFO_DATA_0) then
                    int_CH_INFO(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_CH_INFO(7 downto 0));
                end if;
            end if;
        end if;
    end process;
