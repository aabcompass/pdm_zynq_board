...
void XScurve_adder_Set_CH_INFO(XScurve_adder *InstancePtr, u32 Data) {
    Xil_AssertVoid(InstancePtr != NULL);
    Xil_AssertVoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    XScurve_adder_WriteReg(InstancePtr->Ctrl_bus_BaseAddress, XSCURVE_ADDER_CTRL_BUS_ADDR_CH_INFO_DATA, Data);
}

u32 XScurve_adder_Get_CH_INFO(XScurve_adder *InstancePtr) {
    u32 Data;

    Xil_AssertNonvoid(InstancePtr != NULL);
    Xil_AssertNonvoid(InstancePtr->IsReady == XIL_COMPONENT_IS_READY);

    Data = XScurve_adder_ReadReg(InstancePtr->Ctrl_bus_BaseAddress, XSCURVE_ADDER_CTRL_BUS_ADDR_CH_INFO_DATA);
    return Data;
}
